...
*/
  ...
  printf("The number of names in the list is %d\n", count);
  printf("\n");
}
...
*/
  if (first == NULL)
    ...
    while (first->next != NULL)
      first = first->next;
    printf ("The last element of the list is %s\n", first->data);
  }
  printf ("\n");
}
...
void putFirst (struct node ** firstPtr) {
   pre-condition:  first designates the first node of a list 
     ...
     and placed at the beginning of the list
  char str[100];
  printf ("Enter name to be searched and put in front: ");
  scanf ("%s", str);
  struct node * temp = *firstPtr;
  if ((*firstPtr) == NULL)
    printf ("You are so NULL!!!\n");
  if (((*firstPtr)->next) != NULL) {
    
    while ((strcmp (str, ((*firstPtr)->next)->data) != 0) && ((*firstPtr)->next) != NULL))
    (*firstPtr) = (*firstPtr)->next;
